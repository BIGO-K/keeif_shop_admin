...
mm.ready = function(__callback) {
	...
	mm.event.on(document, 'DOMContentLoaded', __callback, { _isOnce: false });
...
mm.scroll = (function() {
	...
	var base = {
		...
		toggle: function(__is) {
			...
			if (mm.class.some($html, ['S=bom', 'S=modal'])) return;// BOM 이나 모달이 있으면 false로 유지
...
mm.carousel = (function() {
	...
	var initial = {
		...
		_classOn: 'S=carousel-on', // ? :string
		_classClone: 'S=carousel-clone', // ? :string
		pagination: {
	...
	var base = {
		...
		update: function(__$element) {
			...
			__$element.classList.add('S=carousel-ready');// update가 적용될 때 figcaption 노출
...
mm.slider = (function() {
	...
	var initial = {
		...
		_classIntersecting: 'S=slider-intersecting', // ? :string - 영역 내 intersecting 된 전체 요소
		_classOn: 'S=slider-on', // ? :string - 영역 내 온전히(ratio 1) 노출된 전체 요소
		_classClone: 'S=slider-clone', // ? :string
		pagination: {
			_isInner: true, // ? :boolean - 페이지네이션 버튼을 ui 안에서 찾기
			_el: '.btn_slider-page',
		},
	...
	var base = {
		...
		update: function(__$element) {
			...
			__$element.classList.add('S=slider-ready');// update가 적용될 때 figcaption 노출
...
mm.form = (function() {
	...
	var base = {
		...
		// 인터페이스 setter 함수 변경
		changeInterface: function(__interface, __properties) {

			for (const _prop of __properties) {
				if (_prop === `checked`) Object.defineProperty(__interface.prototype, `checkedOnly`, Object.getOwnPropertyDescriptor(__interface.prototype, _prop));// 전체, 부분에서 checked 변경 시 property 이벤트 이슈로 이벤트가 없는 속성 추가

				const descriptor = Object.getOwnPropertyDescriptor(__interface.prototype, _prop);
				const originalSet = descriptor.set;

				descriptor.set = function(__value) {

					originalSet.apply(this, arguments);

					if (this.tagName === `SELECT` && this.selectedIndex < 0) this.selectedIndex = 0;
					if (mm != null) mm.event.dispatch((this.tagName === `OPTION`) ? this.parentElement : this, `property`);
	...
	(function() {
		...
		base.changeInterface(HTMLInputElement, [`checked`, `value`]);
		base.changeInterface(HTMLSelectElement, [`value`, `selectedIndex`]);
		base.changeInterface(HTMLOptionElement, [`selected`]);
	...
	return {
		...
		value: function(__elements, __value) {
			...
			// ~ mm.event.dispatch($elements, 'change');// 요소의 setter 함수에 직접 적용
...
mm.form.text = (function() {
	...
	var initial = {
		...
		_classOff: 'S=off', // ? :string - value 값이 있고, readonly, disabled 상태일 때 클래스
		...
		autocomplete: function(__$text) {
			...
			function keyDownFocus(__e, __$el) {
				...
				mm.delay.on(function() {
					...
					__$text.value = _.last(mm.find('b:not(.text_date)', __$el)).innerText;
			...
			mm.event.on(__$text, 'change keyup', function(__e) {
				...
				switch (__e.type) {
					...
					case 'keyup':
						...
						if (this.value.trim().length > 0) data.__.$autocomplete.classList.add(_classAuto);
						else data.__.$autocomplete.classList.remove(_classAuto);
						break;
				}
			...
			mm.event.on($ui, 'keydown mouseover mouseenter mouseleave focusin focusout', function(__e) {
				...
				if (!data.__.$autocomplete.classList.contains(_classAuto)) return;
				...
				switch (__e.type) {
					...
					case 'focusout':
						mm.delay.on(function() {
							...
							data.__.$autocomplete.classList.remove(_classAuto);
	...
	(function() {
		...
		// 텍스트 입력 (플레이스홀더)
		mm.delegate.on(document, mm.selector(base._dataName, '[]'), 'update change paste keydown input focusout property', function(__e) {
			...
			switch (__e.type) {
				...
				case 'update':
					...
					// textarea 고정 높이
					if ($text.tagName === 'TEXTAREA' && data._resizeMin != null && data._resizeMin > 0) {
						...
						var _lineNum = ($text.scrollHeight === $text.offsetHeight) ? data._resizeMin : Math.ceil(($text.scrollHeight - _padding) / _lineHeight);// 1줄일 때 min을 1로 적용 (scrollHeight === offsetHeight 같음);
...
mm.form.check = (function() {
	...
	var base = {
		...
		checkGroup: function(__$groups, __group) {
			...
			if ($all) {
				...
				if (_count < _min) $all.checkedOnly = false;// * checkedOnly
				else $all.checkedOnly = true;// * checkedOnly
	...
	(function() {
		...
		mm.delegate.on(document, mm.selector(base._dataName, '[]'), 'update change property', function(__e) {
				...
				if (data._type === 'check_all') {
					_.forEach($groups, function(__$group) {
						...
						if (__$group !== $check && __$group.checked !== $check.checked) {
							...
							if (__e.type === 'update') {
								if ($check.checked) __$group.checkedOnly = true;// * checkedOnly
							}
							else __$group.checkedOnly = $check.checked;// * checkedOnly
...
mm.form.file = (function() {
	...
	var initial = {
		...
		image: {
			...
			_imageSize: 'fit', // ? :string - contain, cover, full, fit, block
			...
			_default: null, // ? :string - 초기 값
			_quality: 95, // ? :number - 이미지 퀄리티(100%로 하면 원본 이미지보다 높아져 95로 조정)
			onChange: null, // ? :function
			onChangeParams: [], // ? :array - onChange의 1번째 이후 아규먼트로 전달
			onCancel: null, // ? :function
			onCancelParams: [], // ? :array - onCancel의 1번째 이후 아규먼트로 전달
	...
	var base = {
		...
		load: function(__$file, __callback) {
			...
			if (__$file.closest('.mm_form-image')) {
				...
				loadImage(target, function(__$canvas, __meta) {
					...
					data.file.result = __$canvas.toDataURL(data.file.type, data._quality);
		...
		appendImage: function(__$file, __$canvas) {
			...
			// 고정비율
			var _resizeRatio = data._imageRatio;
			if (typeof(_resizeRatio === 'string')) _resizeRatio = new Function(mm.string.template('return ${RATIO}', { RATIO: _resizeRatio }))();
			if (Number.isFinite(_resizeRatio)) {
				if (['fit', 'full'].includes(data._imageSize)) data._imageSize = 'cover';
				$ui.classList.add('S=image-ratio');

				if (_resizeRatio < 1) mm.element.style($ui, { 'width': mm.number.unit($ui.offsetWidth * _resizeRatio, 'px') });
				mm.element.style($preview, { 'padding-top': mm.number.unit(1 / _resizeRatio * 100, '%') });
			}

			var positions = data._imagePosition.split(' ');
			var _ratio = __$canvas.width / __$canvas.height;
			var _viewRatio = (typeof(_resizeRatio) === 'number') ? _resizeRatio : $preview.offsetWidth / $preview.offsetHeight;
			...
			var caseDraw = {
				fit: () => {
					...
					mm.element.style($ui, { 'width': '' });
					mm.element.style($preview, { 'padding-top': '' });
				...
				block: () => {
					...
					// data._imageRatio에 맞는 리사이징 크롭된 이미지 저장
					// 블록 이미지 저장 시 이미지 영역 기준 리사이징 된 이미지 저장을 위해 사용
					// 이미지가 영역보다 작으면 원본으로 저장
					// 영역보다 크면 가로/세로 중 가까운 비율로 리사이징(cover) 후 가로가 넘어가면 리사이징 된 전체, 세로가 넘어가면 크롭하여 저장
					// 이미지가 영역보다 가로가 크면 원본으로, 세로가 크면 크롭하여 저장
					// imagePosition과 imageSize는 적용되지 않음
					// 영역에 cover 또는 auto 100%로 노출
					if (typeof (data._imageRatio) !== 'string' || !data._imageRatio.includes('/')) return caseDraw.fit();

					var [_width, _height] = data._imageRatio.split('/').map(__value => parseFloat(__value));
					if (__$canvas.height <= _height) caseDraw.fit();// width 상관없이 height 작음
					else {
						// width 작음
						if (__$canvas.width <= _width) {
							$viewCanvas.height = _height;
							viewContext.drawImage(__$canvas, 0, ($viewCanvas.height - __$canvas.height) / 2);

							var _hRatio = $viewCanvas.width / $viewCanvas.height;
							mm.element.style($ui, { 'width': mm.number.unit($ui.offsetWidth * _hRatio, 'px') });
							mm.element.style($preview, { 'padding-top': mm.number.unit(1 / _hRatio * 100, '%') });
						...
						// width 큼
							...
							var _wScale = _width / __$canvas.width;
							var _hScale = _height / __$canvas.height;
							var _isNearWidth = (Math.abs(_wScale - 1) < Math.abs(_hScale - 1));// 가로가 가까움(세로가 큼)
							var _scale = (_isNearWidth) ? _wScale : _hScale;
							var _drawWidth = _width / _scale;
							var _drawHeight = _height / _scale;

							// height 큼(크롭)
							if (_isNearWidth) {
								$viewCanvas.width = _width;
								$viewCanvas.height = _height;
								viewContext.drawImage(__$canvas, (__$canvas.width - _drawWidth) / 2, (__$canvas.height - _drawHeight) / 2, _drawWidth, _drawHeight, 0, 0, _width, _height);
							...
							// width 큼(리사이징)
								...
								$viewCanvas.width *= _scale;
								$viewCanvas.height = _height;
								viewContext.drawImage(__$canvas, 0, 0, __$canvas.width, __$canvas.height, 0, 0, $viewCanvas.width, $viewCanvas.height);
			...
			caseDraw[data._imageSize]();

			data.file.view = {
				result: $viewCanvas.toDataURL(data.file.type, data._quality),
				width: $viewCanvas.width,
				height: $viewCanvas.height
			};
			$viewCanvas.remove();
			// $preview.parentElement.after($viewCanvas);
...
mm.form.multiple = (function() {
	...
	var base = {
		...
		checkButton: function(__$multiple) {
			...
			if (mm.find('.S=image-on', $ui).length > 1) {
				if ($ui.classList.contains('S=multiple-sortable')) mm.element.show(mm.find('.mm_form-multiple-sortable', $ui));
				else mm.element.show(mm.find('.btn_remove-all, .mm_form-multiple-sortable', $ui));
			}
...
mm.bom = (function() {
	...
	var base = {
		...
		appendItem: function(__text, __title, __buttons) {
			...
			document.documentElement.classList.add('S=bom');
		...
		closeItem: function(__callback, __params, __isClose) {
			...
			if (__isClose !== false) {
				...
				// BOM영역 삭제 step.1
					...
					document.documentElement.classList.remove('S=bom');
				}
			}
	...
	return {
		...
		resize: function() {
			...
			if (mm._isPopup) {
				...
				// 리사이즈 과정에서 innerWidth 또는 innerHeight가 변경되지 않는 경우 resize 재실행합니다.
...
mm.modal = (function() {
	...
	var base = {
		$modal: null,
		get _classOn() { return 'S=modal-on'; },
		get _classOld() { return 'S=modal-old'; }, // 이전 모달
		//- 오프너(window)
		get opener() {
...
mm.datepicker = (function() {
	...
	var initial = {
		// 플러그인 옵션
		config: {
			weekdays: ['일', '월', '화', '수', '목', '금', '토'], // ? :string - 일, 월, 화, 수, 목, 금, 토 순으로 표시할 요일
	...
	var base = {
		...
		create: function(__data, __date, __$datepicker) {
			...
			if (!__data.__._isOpen) {
				...
				if (__data.__._periodEnd || __data.__._periodStart || __data.config._disableBeforeDate || __data.config._disableAfterDate) {
					...
					if (__data.config._disableBeforeDate > _today || __data.config._disableAfterDate < _today) $btnToday.disabled = true;
				}

				var $controls = mm.element.create(mm.string.template([
					'<button type="button" class="btn_prev-month"><i class="ico_datepicker-prev-month"></i><b class="mm_ir-blind">이전 달</b></button>',
					'<button type="button" class="btn_next-month"><i class="ico_datepicker-next-month"></i><b class="mm_ir-blind">다음 달</b></button>',
					'<button type="button" class="btn_prev-year"><i class="ico_datepicker-prev-year"></i><b class="mm_ir-blind">1년전</b></button>',
					'<button type="button" class="btn_next-year"><i class="ico_datepicker-next-year"></i><b class="mm_ir-blind">1년후</b></button>'
			...
			_.forEach(mm.find('.mm_datepicker-item', $datepicker), function(__$item, __index) {
				...
				var $row;
				let _satIndex = 5 - (__data.config._firstDay - 1);// 토요일
				let _sunIndex = (_satIndex === 6) ? 0 : _satIndex + 1;// 일요일
				...
				_.forEach(dates, function(__date, __index) {
					...
					if (__data.config._isMonth) {
						...
						if (__index % 7 === _satIndex) $btnDate.classList.add(`S=sat`);
						else if (__index % 7 === _sunIndex) $btnDate.classList.add(`S=sun`);
					}
					...
					if (__data.config._disableBeforeDate || __data.config._disableAfterDate) {
						...
						$btnDate.disabled = (_disableBefore && _disableBefore > _dateString) || (_disableAfter && _disableAfter < _dateString);
					}
		...
		open: function(__$element) {
			...
			mm.event.on(__$element, 'keydown focusout', function checkDatepickerInlineHandler(__e) {

				switch (__e.type) {
					case 'keydown':
						if (__e.keyCode === 9) base.close(); // tab 이동시 닫기
						break;
					case 'focusout':
						if (mm.is.empty(__$element.value)) return false;
	...
	return {
		...
		// 비활성 변경
		disableDate: function(__elements, __option) {

			// ? __elements:element
			// ? option._before:string - 지정한 날짜 이전 비활설
			// ? option._after:string - 지정한 날짜 이후 비활성
			...
			_.forEach($elements, function(__$el) {
				...
				var data = mm.data.get(__$el, base._dataName);
				data.config._disableBeforeDate = (option._before) ? base.checkFormat(option._before, data) : null;
				data.config._disableAfterDate = (option._after) ? base.checkFormat(option._after, data) : null;
